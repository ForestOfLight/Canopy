...
describe('playeractionCommand', () => {
    ...
    it('returns failure when the simplayer is not online', () => {
        ...
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK);
    ...
    it('queues a once action with ONCE timing (default)', () => {
        ...
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.ONCE);
    ...
    it('returns failure for AFTER timing without ticks', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.AFTER, undefined);
        expect(result.status).toBe(CustomCommandStatus.Failure);
    });

    it('queues a delayed once action with AFTER timing', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.AFTER, 10);
    ...
    it('queues a repeating action with CONTINUOUS timing', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.CONTINUOUS);
    ...
    it('returns failure for INTERVAL timing without ticks', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.INTERVAL, undefined);
        expect(result.status).toBe(CustomCommandStatus.Failure);
    });

    it('queues an interval repeating action with INTERVAL timing', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.INTERVAL, 20);
    ...
    it('removes a repeating action with STOP timing', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, TIMING_OPTIONS.STOP);
    ...
    it('returns failure for invalid timing option', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        const result = playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK, 'invalid');
        expect(result.status).toBe(CustomCommandStatus.Failure);
    });

    it('defaults to ONCE timing when no timing option is provided', () => {
        vi.mocked(Understudies.get).mockReturnValue(mockUnderstudy);
        playeractionCommand.playeractionCommand(undefined, 'TestBot', REPEATABLE_ACTIONS.ATTACK);
        expect(mockActions.once).toHaveBeenCalledWith(REPEATABLE_ACTIONS.ATTACK);
    });
});
